...
const parseZoneRule = (data: string): ZoneRule | null => {
  ...
  return ZoneRuleZod.parse(
    rules.split(";").reduce((all, curr) => {
...
const parseTimeZone = (data: string): TimeZone => {
  const standard = parseZone(
    data.match(/BEGIN:STANDARD.*?END:STANDARD/gs)?.[0] ?? ""
  );
  const daylight = parseZone(
    data.match(/BEGIN:DAYLIGHT.*?END:DAYLIGHT/gs)?.[0] ?? ""
  );
  return {
...
const parseEvent = (data: string): CalendarEvent => {
  ...
  return CalendarEventZod.parse(
    lines.reduce((all, curr) => {
      const semicolonLocation = curr.indexOf(";");
      const colonLocation = curr.indexOf(":");
      const [key, ...partedValue] =
        semicolonLocation > -1 && semicolonLocation < colonLocation
          ? curr.split(";")
          : curr.split(":");
...
const parseCalendar = (data: string): Calendar => {
  const timezones = data.match(/BEGIN:VTIMEZONE.*?END:VTIMEZONE/gs);
  const events = data.match(/BEGIN:VEVENT.*?END:VEVENT/gs);
  const restOfTheKeys = Object.keys(CalendarZod.strict().shape).filter(
    (key) => !["timezones", "events"].includes(key.toLowerCase())
  );
  ...
  return CalendarZod.parse({
    ...
    ...restOfTheKeys.reduce((all, curr) => {
      return {
        ...
        [curr]:
          data
            .match(new RegExp(String.raw`${curr.toUpperCase()}:.*`, "g"))?.[0]
            .replace(`${curr.toUpperCase()}:`, "") ?? "",
      };
    }, {} as Record<string, string>),
  });
};
...
const parse = (data: string): Calendar[] => {
  const calendars = data.match(/BEGIN:VCALENDAR.*?END:VCALENDAR/gs);
  if (!calendars) {
    return [];
  }
