...
export const execute = async () => {
    try {
        ...
        const { data: { files: luzonDirectory } } = await gDrive.files.list({ pageSize: 10, fields: "files(id,name,parents)", q: `mimeType = 'application/vnd.google-apps.folder' and '${root.data.id}' in parents and name = 'Luzon'` })
        const luzonDirectoryId = luzonDirectory?.[0].id
        if (!luzonDirectoryId) {
            console.log("Luzon directory not found.")
            return
        }

        const { data: { files: dataFile } } = await gDrive.files.list({ pageSize: 10, fields: "files(id,name,parents)", q: `'${luzonDirectoryId}' in parents and name = 'data.json'` })
        const dataFileId = dataFile?.[0].id
        if (!dataFileId) {
            console.log("Luzon/data.json not found.")
            return
        }

        const { data: { files: configFile } } = await gDrive.files.list({ pageSize: 10, fields: "files(id,name,parents)", q: `'${luzonDirectoryId}' in parents and name = 'config.json'` })
        const configFileId = configFile?.[0].id
        if (!configFileId) {
            console.log("Luzon/config.json not found.")
            return
        }
        ...
        for (const event of outlookEvents) {
            ...
            const matchingGoogleEvent = googleEvents.find(googleEvent => {
                ...
                if (event.isAllDay) {
                    console.log(" ->", googleEvent.end.date, format(asiaJerusalem(event.end), "yyyy-MM-dd"))
                }
